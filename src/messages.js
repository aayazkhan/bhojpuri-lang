...
// All user-facing error text, in Bhojpuri written in Roman letters. Types are named the way `kism`
// names them (sankhya, shabd, list, kosh, ...).
//
// Wording rules, so new messages match:
// - Ask with the same polite "-a" form as the keywords (`maan la`, `lauta da`): likha, badla, chalawa,
//   not the curt likh, badal, chalaw.
// - Bhojpuri words over Hindi ones: kauno/kehu (not koi), kabhio (not kabhi), kuchhu, jaada, apna.
// - "Only" is "sirf": `khaali` is the keyword for nothing, and `bas` would read like `bas kara`.
export const MSG = {
  ...
  codeAfterEnd: () => `${q(K.PROGRAM_END)} ke baad kuchhu na likhe ke chahi.`,
  expected: (want, got) => `Ihaan ${want} chahi rahe, lekin ${got} mil gail.`,
  unknownChar: (ch) => `${q(ch)} ka ha? Samajh me na aail.`,
  unterminatedString: () => `String band na bhail — aakhri quote lagawe ke bhula gaila ka?`,
  unterminatedTemplateBrace: () => `${q("{")} band na bhail — ${q("}")} lagawe ke bhula gaila ka? (Sidha ${q("{")} likhe khatir ${q("\\{")} likha.)`,
  emptyTemplateExpression: () => `${q("{ }")} ke bhitar kuchhu likha, jaise ${q("{naam}")}.`,
  ...
  duplicateParam: (name) => `${q(name)} naam du baar likhal gail ba.`,

  // `hint` is a similar name that does exist, when the name looks like a typo.
  notDeclared: (name, hint) =>
    hint
      ? `${q(name)} naam ke kauno variable na ba. Kahin ${q(hint)} ta na?`
      : `${q(name)} naam ke kauno variable na ba. Pahile ${q(`${K.LET} ${name}`)} likh ke banawa.`,
  alreadyDeclared: (name) =>
    `${q(name)} pahile se banal ba. Nai value dewe khatir ${q(`${name} = ...`)} likha.`,
  ...
  tooManyIterations: (n) =>
    `Loop ${n} baar se jaada chal gail. Kahin i loop kabhio khatam na hoi ka?`,
  tooDeep: () => `Kaam apna-aap ke bahut gahiraai tak bolawat gail. Kahin recursion kabhio khatam na hoi ka?`,
  ...
  noInput: (name) => `Ihaan ${q(name)} ke jawab dewe wala kehu na ba (input na mil sakela).`,
  ...
  noFiles: (kw) => `Ihaan ${q(kw)} na chal sakela: dusra file padhe ke kauno intazaam na ba. Terminal me ${q("bhojpuri")} se chalawa.`,
  fileNotFound: (path) => `${q(path)} file na mil paail.`,
  circularImport: (path) => `${q(path)} ghuma-phira ke apna-aap ke le aawat ba. Duno file ek dusra ke na le aa sakela.`,
  importClash: (name, path) => `${q(name)} pahile se banal ba, aur ${q(path)} bhi ek ${q(name)} det ba. Ek ke naam badla.`,
  ...
  missingKey: (key, hint) =>
    hint
      ...
      : `Kosh me ${key} chaabi na ba. Pahile ${q("ba(kosh, chaabi)")} se jaanch la.`,
